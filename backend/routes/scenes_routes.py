...
def _generate_code(scene_group, project_name):
    ...
    lines.append('')
    lines.append('# AppLocker workaround')
    lines.append('_fake = type(sys)(\"_fake\");')
    lines.append('for _m in [\"av\",\"av.audio\",\"av.audio.frame\",\"av.audio.layout\",\"av.audio.codeccontext\"]:')
    lines.append('    if _m not in sys.modules: sys.modules[_m] = _fake')
    lines.append('')
